...
func (c *Client) CreateOrUpdate(ctx context.Context, u domain.GateWay) (string, error) {
	userName := u.UserName
	if userName == "" {
		userName = fmt.Sprintf("tg_%d", u.TelegranID)
	}
	...
	if err != nil {
		return "", fmt.Errorf("Can not get a response: %w", err)
	}
...
func (c *Client) Disable(ctx context.Context, userName string) error {
	req, err := http.NewRequestWithContext(ctx, http.MethodPut, fmt.Sprintf("%s/api/user/%s", c.baseURl, userName),
